...
-->
## Predator population and herring-predatpr relationship models

- Lots of data analysis and modeling

```{r, message = FALSE, warning = FALSE, fig.align='left', fig.show='hold', out.width='0.49\\linewidth'}
...
-->
```{r, eval=sg_figs,  message=FALSE, warning=FALSE,fig.align='right', fig.show='hold', out.width='0.49\\linewidth'}
...
```{r, eval=sg_figs, message = FALSE, warning = FALSE, fig.show='hold', out.width='0.49\\linewidth'}
...
Nrulesgoodternherrfish <- allres %>%
  filter(CR %in% c("BB", "BB3yr")) %>%
  filter(MedPropYrs_goodProd_Targplustern>0.9 & MedSSBrelSSBmsy>0.9 & YieldrelMSY>0.9 & PropClosure < 0.1) %>%
  group_by(OM,CR)%>%
  summarize(ternfishherr90 = n(), 
            minF = min(FracFtarg), 
            maxF = max(FracFtarg),
