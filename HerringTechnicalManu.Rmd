...
Statistical evidence that $\beta=0$ is evidence of dis-equilibrium (nonstationarity) of NR while statistical rejections of $H_0: \beta=0$ in favor of $H_A: \beta<0$ is evidence of equilibrium (stationarity).  The results of these tests are summarized in two ways.  First, we use the unweighted Z-transform method from the meta-analysis literature to combine the results of these simulations [@stouffer1949american; @Whitlock2005CombiningApproach]. This allows for a test of the  null hypothesis that a particular control rule implemented on a particular operating model does not produce a stable equilibrium. Defining $\phi$ as the standard normal cumulative distribution function, we construct:
\begin{align}
Z  &= \frac{\sum_{i=1}^k \phi^{-1} (1-p_i)}{\sqrt(k)}
\end{align}
$Z$ has a standard normal distribution under the null hypothesis of dis-equilibrium.  We define the performance metric *Equilibrium1* as the $p$-value associated with rejecting the null hypothesis that particular control rule leads to dis-equilibrium of net revenue. Small values of *Equilibrium1* are evidence of a stable equilbrium.  As a robustness check, we define the performance metric *Equilibrium2* as the percentage of simulations in which we reject the null $H_0: \beta=0$ from equation \ref{DF_estimated}.   Large values of *Equilibrium2* are evidence of a stable equilbrium.
...
Catch in the economic model can be different from the yield (from the herring model). This is frequently handled as symmetric implimentation error in the fisheries literature (as it is in this model). However, the economic model suggest a assymetric error term in which the error depends on prices of inputs and outputs. 
...
```{r revenuestability, fig.cap="Tradeoff between herring fleet revenue and stability for HighSlowCorrect operating model and biomass based control rules with 1, 3, or 5 year quota blocks\\label{revenuestability}", message = FALSE, warning = FALSE}
...
ggplot(HSCOM_BBsonly, aes(x=p50_NR, y=stouffer, colour=CR)) + 
  ...
  labs(y="stability", x="Net revenue")
