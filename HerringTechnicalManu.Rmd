...
Several of the herring performance metrics were redundant and exhibited similar tradeoffs.  So, only results for metrics that are commonly of interest were reported further.  Namely, $\frac{yield}{MSY}$, $\frac{SSB}{SSB_{MSY}}$, *IAV*, the frequncy that *Q*=0, and the frequncy that $SSB < \frac{SSB_{MSY}}{2}$.

## Effect of Operating Models

Many performance metrics were characterized by minimal variation in outcomes across operating model, harvest control rule type, or both.  For example, the XXX for Dogfish, tuna, and XXX were relatively insensitive to both Operating Model and HCR type (Figure YAY).  Other performance metrics were highly correlated, like Net Revenue and Yield/MSY (Figure XY-scatter).  A second finding was that one characteristic of the operating models (growth) had minimal effects on all metrics, particularly relative to the effects of *M* and steepness (INSERT A PLOT - maybe boxplot among all CRs?). Furthermore, performance of control rules was similar between the operating models with different *M* and steepness values when reported relative to biological reference points (e.g., $\frac{SSB}{SSB_{MSY}}$, $\frac{yield}{MSY}$), but the scale of results in absolute values was generally different (INSERT PLOT).  Consequently, results were only reported in relative units for the low *M*/high steepness, and slow growth operating model, with the understanding that the operating models differ in meaningful ways (e.g., different *MSY*) if metrics are reported in absolute units.

A third finding was that two classes of control rules (Constant Catch and the 3-year biomass based with a limit on adjustment) performed well in the stability metrics at a large cost of foregone yield.  Our Council didn't want that, and so I think we just say that our Council eliminated them and so will we.  Ultimately, we will no longer look at CC, CCC, or 15% restriction.  Here’s a few results that illustrate that finding (Figure ZZZ). Perhaps stationarity goes here?

We therefore focus the remainder of this section on describing performance metrics that were particularly senstive to changes in control rules or operating models.

## Tradeoffs 

### Herring

  So, only a few tradeoffs that are commonly of interest were reported.  Namely, $\frac{yield}{MSY}$ vs. $\frac{SSB}{SSB_{MSY}}$, $\frac{yield}{MSY}$ vs. *IAV*, $\frac{yield}{MSY}$ vs. the frequncy that *Q*=0, and $\frac{yield}{MSY}$ vs. the frequncy that $SSB < \frac{SSB_{MSY}}{2}$.
