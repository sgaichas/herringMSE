...
|	$y$	|	year
|	$a$	|	age
|	$R_y$	|	herring recruitment
|	$SSB_y$	|	herring spawning stock biomass
|	$h$	|	steepness
|	$\\varepsilon_{R,y}$	|	recrutment process error
|	$\\sigma_R^2$	|	variance of recruitment process error
|	$\\omega$	|	autocorrelation of recruitment error
|	$N_{a,y}$	|	herring abundance
|	$m_a$	|	herring maturity
|	$W_a$	|	herring weight
|	$\\widehat{N}_{a,y}$	|	assessed herring abundance
...
|	$B_y$	|	herring total biomass
|	$\\widehat{SSB}_y$	|	assessed herring spawning stock biomass
|	$\\widehat{B}_y$	|	assessed herring total biomass
|	$M_a$	|	herring natural mortality
| $Y_y$ | herring yield
...
|	$\\tilde{F}_y$	|	target herring fishing mortality
|	$S_a$	|	herring fishery selectivity
|	$\\bar{F}_y$	|	fishing mortality that would remove quota from herring population
|	$Q_y$	|	quota for herring fishery
|	$F_y$	|	realized herring fishery mortality
...
|	$N^P_y$	|	predator abundance
|	$S^P_y$	|	annual predator survival
|	$R^P_y$	|	predator recruitment from Beverton-Holt relationship
|	$\\bar{R}^P_y$	|	predator recruitment as modified by a relationship with herring
|	$v_y$	|	annual predator natural mortality
|	$u$	|	annual predator exploitation mortality
|	$Fw_{int}$	|	intercept of Ford-Walford relationship
|	$Fw_{slope}$	|	slope of Ford-Walford relationship
|	$B^P_y$	|	predator biomass
| $\\gamma$ | multiplier relating herring abundance threshold to predator recruitment
|	$N_{thresh}$	|	threshold in herring abundance that induces a reaction from a predator
|	$\\alpha_y^P$	|	intercept of Ford-Walford relationship as modified by a relationship with herring
|	$\\rho_y^P$	|	slope of Ford-Walford relationship as modified by a relationship with herring
|	$\\delta$	|	exponent relating herring abundance to predator survival
|	$\\bar{W}_{y}$	|	average weight of herring
...
|	$a_i$	|	input to price and landing model
|	b	|	input to price and landing model
|	$\\gamma$	|	input to price and landing model
|	$\\alpha$	|	input to price and landing model
|	$\\theta_i$	|	input to price and landing model
|	$\\beta$	|	input to price and landing model
|	$\\xi$	|	input to economic stationarity metric
"
...
| Base annual nat. mortality rate *v* | 0.14         | 0.1          | 0.092        |
| Annual exploitation rate *u*  | 0.079        | 0.00         | 0.092        |
| Growth intercept $Fw_{int}$ | 0.020605     | 0.00015      | 0.000278     |
| Growth slope $Fw_{slope}$  | 0.9675       | 0.0          | 0.9577       |
| Initial abundance $N^P_1$ (n for fish, n nesting pairs for tern)        | 111864       | 3000         | 49629630     |
| Initial biomass $B^P_1$ (metric tons for fish, kg for tern but not used in model)        | 27966        | 1.5          | 134000       |
| Recruit delay (age, years)  | 1            | 4            | 10           |
| Prey-recruitment link $\\gamma$   | 1 (off)      | 1.09         | 1 (off)      |
| Prey-mortality link $\\delta$     | 0 (off)      | 0 (off)      | 0.2          |
| Prey-growth link  $\\lambda$       | 1.1          | 1 (off)      | 1 (off)      |
"
cat(tabl) # output the table in a format good for HTML/PDF/docx conversion
```
