...
Catch in the economic model can be different from both quota and yield (from the herring model). This is frequently handled as symmetric implementation error in the fisheries literature (as it is in this model). However, the economic model suggests that either an assymetric error term, inwhich the error depends on prices of inputs and outputs, or a more integrated biological and economic model is warranted. 
...
In order to meet management timelines, the development of the herring, predator, and economic models must be done in parallel, not in sequence.  This requires an educated guess about which predators and predator outcomes are sensitive to changes in HCRs; this difficult.  For example, prior to completing the predator models, we did not know that tuna weight but not abundances would be sensitive to various operating models, but not harvest control rules.  Spending research time examining how changes in abundance affect affect costs of harvesting tuna, would have resulted in additional performance measures that would not help  managers select among control rules.

## Making stuffs better. 
