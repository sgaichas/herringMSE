...
For each biomass based control rule variant, a range of values for the three parameters defining the control rule were evaluated.  The proportion ($\psi$) of $F_{MSY}$ that dictates the maximum desired fishing mortality rate was varied from 0.1$F_{MSY}$ to 1.0$F_{MSY}$ in increments of 0.1, while the lower and upper SSB threshold parameters ($SSB_{low} \; SSB_{up}$) were varied from 0.0$SSB_{MSY}$ to 4$SSB_{MSY}$ but with inconsistent increments (i.e., 0.0, 0.1, 0.3, 0.5, 0.7, 0.9, 1.0, 1.1, 1.3, 1.5, 1.7, 2.0, 2.5, 3, 3.5, 4).  The full factorial of combinations for the three biomass based control rule parameters produced 1,360 shapes (note $SSB_{low}$ must be <= $SSB_{up}$) and each of these shapes was evaluated for each of the four biomass based control rule variants described above.
...
In order to meet management timelines, the herring, predator, and economic models were developed in parallel, and not in sequence.  This required an educated guess about which predators and predator metrics/outcomes were sensitive to the range of HCRs.  For example, prior to completing the predator models, we did not know that tuna weight but not abundance would be sensitive to various operating models (i.e., fast and slow herring growth) but not the harvest control rules.  Developing the predator models in sequence may have revealed this result earlier in the process and resulted in efficiency gains. Spending research time examining how changes in herring abundance affect costs of harvesting tuna, would have resulted in additional performance measures that would not help managers select among control rules because herring abundance had relatively little effect on tuna.
...
```{r refpt,tab.cap="Life history traits and reference points including unfished SSB (i.e., virgin SSB), $SSB_{MSY}$, MSY, and $F_{MSY}$ reference points for the Atlantic herring operating models \\label{RefPts}", echo=FALSE, message=FALSE, warnings=FALSE, results='asis' }
tabl <- "
| Steepness | Natural Mortality | Growth | Unfished SSB (mt) | SSB MSY (mt) | MSY (mt) | F MSY |
|:----|:----|:----|:----|:----|:----|:----|
...
```{r popsources, tab.cap="Predator population model specification and parameter sources. See methods for details of Gulf of Maine Seabird Working Group (GOMSWG) data. \\label{predsource}", echo=FALSE, message=FALSE, warnings=FALSE, results='asis'}
tabl <- "
...
| Stock-recruitment   | @iccat_report_2015 and @porch_making_2016 | Derived from GOMSWG data | @rago_implications_1998 and @rago_biological_2010 | No time series data for our region | 
| Natural mortality | @iccat_report_2015 | @nisbet_common_2002 | @rago_update_2013 and Rago pers. comm. 2016 | Derivable from @hayes_us_2017? |
| Fishing mortality | @iccat_report_2015 | n/a | @rago_update_2013 and Rago pers. comm. 2016 | Derivable from @hayes_us_2017? |
| Initial population | @iccat_report_2015 | Derived from GOMSWG data | @rago_update_2013 and Rago pers. comm. 2016 | Derivable from @hayes_us_2017? |
| Weight at age | @restrepo_updated_2010 | @nisbet_common_2002 | @rago_implications_1998 | Depending on species, derivable from literature | 
"
cat(tabl) # output the table in a format good for HTML/PDF/docx conversion
```
...
| Unfished spawning pop (metric tons for fish, n of nesting pairs for tern)   | 6.69E+04     | 45000        | 300000       |
| Steepness *h*            | 1.0          | 0.26         | 0.97         |
| Annual nat. mortality rate *v*| 0.14         | 0.1          | 0.092        |
| Annual exploitation rate *u*  | 0.079        | 0.00         | 0.092        |
| Growth intercept *FWint* | 0.020605     | 0.00015      | 0.000278     |
| Growth slope *FWslope *  | 0.9675       | 0.0          | 0.9577       |
| Initial abundance (n for fish, n nesting pairs for tern)        | 111864       | 3000         | 49629630     |
| Initial biomass  (metric tons for fish, kg for tern but not used in model)        | 27966        | 1.5          | 134000       |
| Recruit delay (age, years) *a*  | 1            | 4            | 10           |
...
```{r, fig.cap="Modeled herring average weight-tuna growth relationship \\label{herringtuna}", message = FALSE, warning = FALSE}
# from MSE_TunaLink.R
...
qplot(preyAvgWt, AnnualAlpha, geom="line", xlab="Herring average weight", ylab="Tuna AnnAlpha parameter") +
  theme_Publication() + scale_colour_Publication()

```
\newpage

```{r, eval=sg_figs, fig.cap="Stock-recruitment function for Gulf of Maine common terns assuming 10\\% fledgling->adult survival \\label{ternSR}", message = FALSE, warning = FALSE}
...
ternrec+
  ...
  labs(x="Number of nesting pairs", y="Number of Recruits (Fledglings/10)") +
  theme_Publication() + scale_colour_Publication()
...
```{r, eval=sg_figs, fig.cap="Gulf of Maine tern annual productivity distributions by majority diet item offered to fledgelings. The black line represents the target tern productivity of 1.0 fledgling per nest. \\label{chickdiet}", message = FALSE, warning = FALSE}
...
dietitem + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(x="Species group", y="Tern productivity")+
  theme_Publication() + scale_colour_Publication()
...
```{r, eval=sg_figs, fig.cap="Herring proporiton in diet and annual tern productivity (fledglings/nest) by Gulf of Maine colony. The black line represents the target tern productivity of 1.0 fledgling per nest. \\label{proddiet}", fig.height=8, fig.width=10,  out.width='\\linewidth', message = FALSE, warning = FALSE}
...
ternproddiet + facet_wrap(~colony) + #theme(text=element_text(size=16))
  labs(x="Proportion of herring in diet", y="Productivity")+
  theme_Publication() + scale_colour_Publication()

```
\newpage

```{r, eval=sg_figs, fig.cap="Modeled influence of herring total biomass on tern reproductive success. The black line represents the target tern productivity of 1.0 fledgling per nest. \\label{herrternmod}", message = FALSE, warning = FALSE}
#ternpopproddiet<-read.csv("/Users/sgaichas/Documents/0_Data/MSE/HerringMSE/GOMTernPopsProdDiet_Corrected.csv")
...
totprodnomonherr + 
  ...
  labs(x="Herring total biomass (metric tons)", y="Tern productivity (fledglings per nesting pair)")+
  theme_Publication() + scale_colour_Publication()
...
```{r, eval=sg_figs, fig.cap="Population trends for Gulf of Maine terns with and without simulated herring-common tern productivity relationship \\label{terntrendwherring}", message = FALSE, warning = FALSE}
#ternpopproddiet<-read.csv("/Users/sgaichas/Documents/0_Data/MSE/HerringMSE/GOMTernPopsProdDiet_Corrected.csv")
...
ternpoptrend +   geom_point() +
  stat_smooth(method = "lm", formula = y ~ x, size = 1) +
  geom_point(data=simTern1, aes(x=yr, y=PredN, colour="Simulated common terns,\nno herring link")) +
  geom_point(data=simTern, aes(x=yr, y=PredN, colour="Simulated common terns,\n herring link")) +
  labs(x="Year", y="Number of nesting pairs")+
  theme_Publication() + scale_colour_Publication()
...
```{r,eval=sg_figs, fig.cap="Annual diet compositions for major groundfish predators of herring estimated from NEFSC food habits database. Herring proportion in diet is highlighted in solid fill. \\label{gfishdiets}", fig.height=8, fig.width=11, out.width='\\linewidth', message = FALSE, warning = FALSE}
...
compplot3 <- ggplot(herrindiet, aes(Year, Percent)) + 
  #geom_bar(stat = "identity", aes(fill=fillwhite))
  geom_bar(stat = "identity", aes(fill=Prey)) 

compplot3 + facet_wrap("Pred", nrow=3)  +
  theme_Publication() + scale_colour_Publication() # + theme(legend.position="none")

```

\newpage

```{r, eval=sg_figs, fig.cap="Relationship of assessed groundfish spawning stock biomass (SSB) with the proportion of herring in diet \\label{gfishBherrdiet}", message = FALSE, warning = FALSE}
...
```{r, eval=sg_figs, fig.cap="Modeled herring relative population size-dogfish natural mortality relationship \\label{herringdogfish}", message = FALSE, warning = FALSE}
...
```{r YAY, fig.cap="Differences between operating models for key metrics\\label{YAY}", fig.height=14, fig.width=11.5, out.width='\\linewidth', message = FALSE, warning = FALSE}
# metrics <- c('stouffer',
#              'stationary',
#              'p50_NR',
#              'PropClosure',
...
HSCOM_BBsonly <- HiSlowCorrectOM %>%
  filter(CR %in% c("BB", "BB3yr", "BB5yr", "BB3yrPerc"))

HSCOM_BBsonly$CR <- factor(HSCOM_BBsonly$CR, levels = c("BB", "BB3yr", "BB5yr", "BB3yrPerc"))
