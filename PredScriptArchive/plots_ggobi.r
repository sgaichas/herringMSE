  ...
  source(paste(direct,"HerringPercentiles.R",sep=""))  ##get herring percentiles
  ...
  allres<-merge(allres,allres25)
  allres<-merge(allres,allres75)
...
TradeOffPlots<-function(xwantb=NULL,ywantb=NULL,xnameb=NULL,ynameb=NULL,wantcolor=NULL,Ywant=NULL,Xwant=NULL){  
    ...
    if(CR. < (length(ctrlrulenames)-1)) {      
      ...
      if(wantcolor==FALSE){
       colors<-"black"
        ...
        if(ywantb %in% lessthans & !xwantb %in% lessthans) {
          colors<-ifelse(allres.sub[,ywantb] <= Ywant & allres.sub[,xwantb] >= Xwant,"green1","black")
        } else { 
          if(!ywantb %in% lessthans & xwantb %in% lessthans) {
            colors<-ifelse(allres.sub[,ywantb] >= Ywant & allres.sub[,xwantb] <= Xwant,"green1","black")
          } else {
...
####################################################
  ...
  ctrlwant=2 #which control desired; number is position in ctrlrulenames
  Hiwant=0.5 #If a BB control rule, then what hi threshold as fraction of SSBmsy
  Lowant=0 #If a BB control rule, then what lo threshold as fraction of SSBmsy
  Fwant=0.9 #If a BB control rule, what is max target F as frac of Fmsy
  ...
  for(i in 1:length(xaxiswant)) {
    ...
    #layout.show(graph)
    TradeOffPlots(xwantb=xaxiswant[i],ywantb=yaxiswant[i],xnameb=xaxisnames[i],ynameb=yaxisnames[i],wantcolor=TRUE,Ywant=yaxisval[i],Xwant=xaxisval[i])
    Tradeplotname<-paste(paste(xaxiswant[i],paste(yaxiswant[i],"_TRADE",sep=""),sep="VS"),OMnames[OMwant],sep="_")
...
#ForCRplot<-allres[allres$FracBmsyThreshHi==0.5 & allres$FracBmsyThreshLo==0.0 & allres$FracFtarg==0.9 & allres$OM==OMnames[2] & allres$CR==ctrlrulenames[2],]  
  ForCRplot<-allres[allres$MedSSBrelSSBzero>0.6 & allres$YieldrelMSY > 0.8 & allres$OM==OMnames[2] & allres$CR==ctrlrulenames[2],]   
...
CRplotfun(FracHi=FracHis[c],FracLo=FracLos[c],maxFtarg=maxFs[c],c=c,newplot=TRUE)
  ...
  savePlot(filename=paste(direct,paste("Plots","BiasedMeetYandSSB.png",sep="\\"),sep=""),type="png")
graphics.off()
