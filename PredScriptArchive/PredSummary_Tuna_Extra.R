...
for(OMtype in OMlist){
  ...
  for(crt in CRtype){
    ...
    for(crnum in CRnum){
      ...
      simMed <- predOut %>%
        ...
        summarize(#medPredB = median(PredB, na.rm=T),
                  ...
                  #nPredB_goodstat = sum(PredB_status>=1, na.rm=T),
      ...
      crMed <- data.frame(CRnum = crnum, 
                          ...
                          #MinYrs_goodBStatus = min(simMed$nPredB_goodstat, na.rm=T),
                          #AvPropYrs_goodBstatus = mean(simMed$nPredB_goodstat, na.rm=T)/wantyears,
                          ...
                          Q25PropYrs_goodAvWtstatus = quantile(simMed$nPredAvWt_goodstat, probs=0.25, na.rm=T)/wantyears,
                          MedPropYrs_goodAvWtstatus = median(simMed$nPredAvWt_goodstat, na.rm=T)/wantyears,
                          Q75PropYrs_goodAvWtstatus = quantile(simMed$nPredAvWt_goodstat, probs=0.75, na.rm=T)/wantyears
    ...
    write.table(Res, paste(PredDirecCR,"/",OMtype,"_",crt,"_","predExtra.txt", sep=""), row.names = F)
