...
#### The groundfish that can not be named pars (see HerringMSEpredspars.xlsx for derivation
# this is based on dogfish see Rago pubs and spreadsheet
# also, female SSB is population, exploitation, and growth pars
...
Predzero      <-  300000 #5.90E+05 # #5.60E+09  #50000 #predator unfished total N or biomass
Predsteep     <-  0.97 #0.97 #0.8 #predator steepness
...
FWalpha       <-  0.000278  #0.243 #ford-walford plot intercept parameter if doing biomass units tons
FWrho         <-  0.9577  #1.45 #ford-walford plot slope parameter if doing biomass units; serves as max if time varying

#Recwt         <-  0.0027  #age 11 not 1  #avg wt at recruitment age, t (wt at PredRecdelay)
#Prerecwt      <-  0.0024  #age 10 not 0   #avg wt at rec age - 1, t

PredN         <-  c() #for predator abundance vector
PredN[1]      <-  49629630  #7000 #initial abundance in year 1
PredB         <-  c() #for predator biomass vector if doing in those units
PredB[1]      <-  134000  #6000 #initial biomass in year 1 
PredRecdelay  <-  10  #1 #delay in years for when recruits in year y are added to population
...
if(DOG){
  SSB_MSY<-159288  #63102
  DOG_MThresh<-2000000 #irrelevant
}
...
OMDirec<-c('//net/mse/')
OMlist<-c('LoM_HiSteep_NoAssBias_OldWt') #redo
...
#          'HiM_LowSteep_AssBias_OldWt','LoM_HiSteep_AssBias_OldWt', 'LoM_HiSteep_AssBias_RecWt', 'HiM_LowSteep_AssBias_RecWt')
#CRtype<-c("BB", "BB3yr", "BB5yr", "BB3yrPerc", "CC", "CCC")
CRtype<-c("BB") #redo, broke
...
for(crt in CRtype){
  ...
  for(crnum in CRnum){
     ...
     #BASE ON TOTAL B? No
     Gfishforage<-cbind(preyB, Yr=rep(1:Nyears$n, max(preychar$Sim)))
    ...
    # BFTforagechar <- left_join(BFTforage, preychar)
     ...
     for (h in 1:max(preyNsim$Sim)){
       ...
       #preyN<-Gfishforage$TotalBio[Ternforage$Sim==h]
       #preyNzero<-DOG_MThresh
       ...
       if(h==1){
         predOut<-data.frame(Sim=h, Yr=c(1:nyears), PredB=PredB, PredN=PredN, PredRec=Predrec[10:159], PredAvWt=PredB/PredN, PredB_status=PredB/SSB_MSY, PredAvWt_status=(PredB/PredN)/Weq )
       } else {
         predOutn<-data.frame(Sim=h, Yr=c(1:nyears), PredB=PredB, PredN=PredN, PredRec=Predrec[10:159], PredAvWt=PredB/PredN, PredB_status=PredB/SSB_MSY, PredAvWt_status=(PredB/PredN)/Weq )
       ...
       if(doplotsbysim){
         ...
         if(numbs_weight != 1){ #only do if biomass units requested
           plot(PredB,type='l',col="black",xlab="Year",ylab="Predator Biomass",lwd=2, ylim=c(0, max(PredB, SSB_MSY))) 
           abline(h=SSB_MSY)
           par(new=T)
           plot(PredB/PredN, type="l", col="blue", axes=F, xlab=NA, ylab=NA, ylim=c(0, max(PredB/PredN, Weq)))
           axis(side = 4)
     ...
     if(dosummaryplots) {
        ...
        plot(predOut$Yr[predOut$Sim==1], predOut$PredB_status[predOut$Sim==1], ylim=c(0,2), type="l", col = rgb(0, 0, 0, 0.3), ylab="PredB/BMSY", xlab="")
            for (j in 1:max(predOut$Sim)){
              lines(predOut$Yr[predOut$Sim==j], predOut$PredB_status[predOut$Sim==j], type="l", col = rgb(0, 0, 0, 0.3))
            }
