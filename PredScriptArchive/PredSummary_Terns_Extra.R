  ...
  for(crt in CRtype){
    ...
    for(crnum in CRnum){
      ...
      crMed <- data.frame(CRnum = crnum, 
                          ...
                          Q25PropYrs_goodProd_Targplus = quantile(simMed$nPredProd_Targplus, probs=0.25, na.rm=T)/wantyears,
                          MedPropYrs_goodProd_Targplus = median(simMed$nPredProd_Targplus, na.rm=T)/wantyears,
                          Q75PropYrs_goodProd_Targplus = quantile(simMed$nPredProd_Targplus, probs=0.75, na.rm=T)/wantyears
