  ...
  for(crt in CRtype){
    ...
    for(crnum in CRnum){
      ...
      crMed <- data.frame(CRnum = crnum, 
                          ...
                          Q25PropYrs_okBstatus = quantile(simMed$nPredB_okstat, probs=0.25, na.rm=T)/wantyears,
                          MedPropYrs_okBstatus = median(simMed$nPredB_okstat, na.rm=T)/wantyears,
                          Q75PropYrs_okBstatus = quantile(simMed$nPredB_okstat, probs=0.75, na.rm=T)/wantyears
